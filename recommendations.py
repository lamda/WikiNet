...
class Recommender(BaseRecommender):
    ...
    def print_examples(self, rec_type, n_examples=100):
        ...
        if rec_type == 'scc_based':
            func, label = self.get_next_recommendation_scc_based, 'SCC-based'
        elif rec_type == 'vc_based':
            func, label = self.get_next_recommendation_vc_based, 'VC-based'

        fpath = os.path.join('data', self.wiki_name, 'recs_' + rec_type + '.txt')
        with io.open(fpath, 'w', encoding='utf-8') as outfile:
            for i in range(n_examples):
                print('\r', i+1, '/', n_examples, end='')
